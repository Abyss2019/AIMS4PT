...
class Dataset:
    try:
        ...
        print("Warning: File not found. Dataset not loaded.")
    try:
        Train_P =  pd.read_excel('datasets/train_dataset_pressure.xlsx')
    except FileNotFoundError:
        Train_P = 'training dataset for cpx and cpx_liquid barometers'
        print("Warning: File not found. Dataset not loaded.")
    try:
        Train_T = pd.read_excel('datasets/train_dataset_temperature.xlsx')
    except FileNotFoundError:
...
def replace_zeros(dataFrame, column_name):
    ...
    if pd.isna(min_non_zero) or min_non_zero <= 0 or np.isinf(min_non_zero):
        min_non_zero = 1e-10
...
def P_T_predictors(output, model):
    ...
    # 例如 Model_cpx_only_P_bias
    filename_prefix = f"Model_{model}_{output[0]}_bias"

    scaler_path = os.path.join(base_dir, f"{filename_prefix}.joblib")
    onnx_path = os.path.join(base_dir, f"{filename_prefix}.onnx")
    json_path = os.path.join(base_dir, f"{filename_prefix}.json")

    scaler = joblib.load(scaler_path)
    predictor = rt.InferenceSession(onnx_path)
