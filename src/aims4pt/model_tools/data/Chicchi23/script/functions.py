...
import sys
# avoid re-importing TensorFlow (and re-registering specs)
...
def easy_predict(data, T_P,  dir):    
    ...
    for e in range(N):
        ...
        inp  = tf.keras.Input(shape=(11,), dtype="float32", name="dense_input")
        out  = layer(inp)
        ...
        # #load modell
        # model = tf.keras.models.load_model(
        #     directory + "/mod_" + names_target + '_' + sect + "/Bootstrap_model_" + str(e) + '.keras', compile=False)
...
# 全局缓存：key -> list[tf.keras.Model]
_MODEL_CACHE = {}

def _get_model_spec(T_P: str):
    """Return (subdir, N, array_max, target_name)."""
    if T_P == "P":
...
def preload_models(model_root: str, T_P: str, sect: str, n_features: int):
    """
    Preload and cache bootstrap models.
...
def easy_predict(data, T_P: str, dir: str):
    """
    Predict mean of bootstrap ensemble. Return mean only.
    Assumes first 4 columns are index/info cols; the rest are features.
    """
    col = data.columns
    index_col = [col[i] for i in range(0, 4)]
    df_noindex = data.drop(columns=index_col)
    ...
    # preload (cached)
    models = preload_models(model_root=dir, T_P=T_P, sect=sect, n_features=n_features)

    # scale factor
    _, N, array_max, _ = _get_model_spec(T_P)
...
def clear_model_cache():
    """If you change models on disk or want to free memory."""
    _MODEL_CACHE.clear()
